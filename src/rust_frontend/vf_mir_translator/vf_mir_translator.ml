  ...
  struct
    ...
    let translate_rvalue (rvalue_cpn : RvalueRd.t) (loc : Ast.loc) =
      ...
      | Ref ref_data_cpn ->
          ...
          let fn_name =
            match place_ty, BorrowKind.get bor_kind_cpn, BodyRd.PlaceKind.get (BodyRd.Place.kind_get place_cpn), is_implicit with
            | `Str, Shared, SharedRef, _ -> "create_str_ref"
            | `Str, Shared, _, _ -> "reborrow_str_ref"
